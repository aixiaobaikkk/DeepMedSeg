...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1,model_name='LUCF_Net'):
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                if model_name == 'LUCF_Net' or model_name == 'PVT_Cascade' or model_name == 'Swin_cat' or model_name == 'Exp_Net_fusion' :
    ...
    if test_save_path is not None:
        ...
        print("saving nii to:",test_save_path)
    return metric_list
...
def val_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1,model_name='LUCF_Net'):
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            net.eval()
            with torch.no_grad():
                if model_name == 'LUCF_Net' or model_name == 'PVT_Cascade' or model_name == 'LUCF_Net_ce_dice'\
                        or model_name == 'LUCF_Net_ce_dice_iou' or model_name == 'LUCF_Net_ce_iou'or model_name == 'LUCF_Net_ce_ohem_iou':
                    p1, p2, p3, p4, = net(input)
                    outputs = p1 + p2 + p3 + p4
                elif model_name == 'Exp_Net':
                    p1, p2, p3, p4, = net(input)
                    outputs = p4
                elif model_name == 'Exp_Net_fusion':
                    p1, p2, p3, p4, = net(input)
                    outputs = p4+p3+p2+p1
...
def update_weights(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        for ind in range(image.shape[0]):
