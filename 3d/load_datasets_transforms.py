...
from monai.transforms import Compose, LoadImaged, AddChanneld, Spacingd, Orientationd, ScaleIntensityRanged, ResizeWithPadOrCropd, CropForegroundd, RandCropByPosNegLabeld, RandShiftIntensityd, RandAffined, ToTensord, Lambda
    ...
    if dataset == 'feta':
        ...
        train_transforms = Compose(
            [
                ...
                #
                # ResizeWithPadOrCropd(keys=["image", "label"], spatial_size=(96, 96, 96), mode='constant'),  ##改变
                Orientationd(keys=["image", "label"], axcodes="RAS"),
                ScaleIntensityRanged(
        ...
        test_transforms = Compose(
            [
                ...
                # ResizeWithPadOrCropd(keys=["image", "label"], spatial_size=(96, 96, 96), mode='constant',allow_missing_keys=False), ##改变
                Orientationd(keys=["image"], axcodes="RAS"),
                ScaleIntensityRanged(
                    keys=["image"], a_min=-125, a_max=275,
                    b_min=0.0, b_max=1.0, clip=True,
